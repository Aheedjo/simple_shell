...
int _unsetenv(char *name)
{
	...
	/* Calculates the length of the environ array. */
	while (environ[environ_len])
		environ_len++;

	/* Allocates memory for a new environ array with one less variable (last element is null) */
	...
	/* Copy the old environ to new_environ, excluding the unset variable. */
