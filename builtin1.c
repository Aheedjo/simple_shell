...
int _setenv(const char *name, const char *value, int overwrite)
{
	...
	/* Error handling: The name must be in the correct format. */
	if (name == NULL || name[0] == '\0' || _strchr(name, '=') != NULL)
		return (-1);

	/* If the environment variable already exists and the overwrite flag is set to false */
	...
	/* Call putenv and store the return value in putenv_status. */
